...
export const useDownloadInvoicePDF = () => {
  ...
  const downloadInvoicePDF = async (order: Order) => {
    ...
    try {
      ...
      pdf.text(`Subtotal: Rs. ${(order.totalAmount).toFixed(2)}`, 20, y);
      y += 6;
      pdf.text(`Shipping: Rs. 0`, 20, y);
      y += 6;
      pdf.setFont('helvetica', 'bold');
      pdf.text(`Total: Rs. ${order.totalAmount.toFixed(2)}`, 20, y);
      y += 10;
      ...
      dispatch(showNotification({ message: `Failed to generate PDF. ${err}`, type: 'error' }));
    } finally {
