...
const WishlistItemCard: React.FC<WishlistItemCardProps> = ({ item }) => {
    ...
    const removeMutation = useMutation({
        ...
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ["wishlistItems"], exact: false }),
    });
        ...
        <div className="w-full flex justify-between items-center border-b border-neutral-200 py-4">
            <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <span className="text-sm text-neutral-500">{item.product.category?.name}</span>
                </div>
            </div>
            ...
            <div className="flex flex-col items-end gap-2 min-w-[100px]">
                {item.product.discountPercent > 0 ? (
                    <div className="flex flex-col items-end">
                        ...
                        <span className="line-through text-neutral-400 text-sm">${item.product.price}</span>
                    </div>
                ) : (
