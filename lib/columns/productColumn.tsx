...
import { PenSquare, Trash, Star, StarHalf, Star as StarOutline } from "lucide-react";
...
const renderStars = (rating: number) => {
    ...
    for (let i = 0; i < fullStars; i++) stars.push(<Star key={`full-${i}`} size={16} className="text-yellow-500" />);
    if (halfStar) stars.push(<StarHalf key="half" size={16} className="text-yellow-500" />);
    while (stars.length < 5) stars.push(<StarOutline key={`empty-${stars.length}`} size={16} className="text-gray-300" />);
...
export const productColumn: ColumnDef<Product>[] = [
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <Image
                    src={url.startsWith("/uploads") ? `${process.env.NEXT_PUBLIC_STATIC_URL}${url}` : url}
    ...
    },
    {
        ...
        cell: ({ row }) =>
            row.original.discountPercent > 0 ? (
                <span className="px-2 py-1 rounded bg-gray-100 text-xs font-medium">{row.original.discountPercent}%</span>
            ) : (
                "—"
            ),
    },
    {
        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-1">
                ...
                <span className="text-sm text-gray-600">{row.original.averageRating.toFixed(1)}</span>
            </div>
        ),
    },
    {
        ...
        cell: ({ row }) =>
            row.original.isFlashSale ? (
                <span className="px-2 py-1 rounded bg-red-500 text-white text-xs font-semibold">Flash Sale</span>
            ) : (
                "—"
            ),
    },
