...
import { addItemToWishlist, checkWishlistItem, removeItemFromWishlist } from '@/lib/api/wishlist';
...
const ProductDetailPage = () => {
    ...
    const { data: wishlistCheck } = useQuery<{ inWishlist: boolean, wishlist: { id: number } }>({
    ...
    const addToWishlistMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['wishlistItems', Number(productId)] });
            dispatch(showNotification({ message: 'Item added to Wishlist successfully', type: 'success' }));
        },

        onError: () => {
            dispatch(showNotification({ message: 'Failed to add Item to Wishlist', type: 'error' }));
        },
    });
    ...
    const removeFromWishlistMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['wishlistItems'] });
            queryClient.invalidateQueries({ queryKey: ['wishlistItems', Number(productId)] });
            dispatch(showNotification({ message: 'Item removed from Wishlist successfully', type: 'success' }));
        },

        onError: () => {
            dispatch(showNotification({ message: 'Failed to remove Item from Wishlist', type: 'error' }));
        },
    });
    ...
    const addToCartMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            dispatch(showNotification({ message: 'Item added to Cart successfully', type: 'success' }));
        },

        onError: () => {
            dispatch(showNotification({ message: 'Failed to add Item to Cart', type: 'error' }));
        },
    });
    ...
    const handleToggleWishlist = () => {
        if (!user) {
            dispatch(showNotification({ message: 'Please log in to use Wishlist', type: 'error' }));
            return;
        }
            ...
            <div className='flex gap-16'>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <p className="mt-4 text-xl font-bold">
                        Rs. {(product.price - (product.price * product.discountPercent) / 100).toLocaleString()}
                        <span className="line-through text-gray-400 ml-2 text-base">
                            Rs. {product.price.toLocaleString()}
                        </span>
                    </p>
                    <p className="mt-2 text-sm text-yellow-600">⭐ {product.averageRating} / 5</p>
                    ...
                    <div className='flex gap-4 items-center mt-2'>
                        ...
                        <button className='cursor-pointer' onClick={handleToggleWishlist} disabled={wishlistPending}>
                            {wishlistPending ? (
                                <Loader2 className="animate-spin text-red-500" size={26} />
                            ) : (
                                <Heart
                                    size={26}
                                    className={`transition duration-300 ${wishlistCheck?.inWishlist ? "text-red-500 fill-red-500" : ""
                                        }`}
                                />
                            )}
                        </button>
