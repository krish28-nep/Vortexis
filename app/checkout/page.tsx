...
const CheckoutPage = () => {
  ...
  const [selectedPayment, setSelectedPayment] = useState<"cod" | "khalti" | null>(null);
  ...
  // Fetch cart items
  const { data: cartItems, isLoading: cartItemsLoading, isError: cartItemsError } = useQuery<CartItem[]>({
    queryKey: ["cartItems"],
  ...
  const addOrderMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      dispatch(showNotification({ message: "Order has been placed successfully!", type: "success" }));
      router.push(`/order-details?orderId=${data.id}`);
    },
    onError: () => {
      dispatch(showNotification({ message: "Failed to create order. Please try again.", type: "error" }));
    },
  });

  // Khalti Payment Mutation
  const khaltiMutation = useMutation({
    mutationFn: async (payload: KhaltiPaymentPayload) => await initiateKhalti(payload),
    onSuccess: (data) => {
      window.location.href = data.payment_url;
    },
    onError: () => {
      dispatch(showNotification({ message: "Failed to initiate Khalti payment", type: "error" }));
    },
  });

  const handleConfirmOrder = () => {
    if (!filteredItems.length) {
      dispatch(showNotification({ message: "No items selected for order", type: "error" }));
      return;
    }
    if (!selectedPayment) {
  ...
  const handleKhaltiPayment = () => {
    if (!filteredItems.length) {
      dispatch(showNotification({ message: "No items selected for payment", type: "error" }));
      return;
    }
    if (!user) {
      dispatch(showNotification({ message: "You must be logged in for Khalti payment", type: "error" }));
      return;
    }
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex flex-col gap-4 flex-1">
          {filteredItems.length > 0 ? (
            filteredItems.map((item) => <CartItemCard key={item.id} variant="checkout" item={item} />)
          ) : (
            <div className="flex flex-col gap-2">
              <p>No items selected for checkout.</p>
              <Button text="Back to Cart" onClick={() => router.push("/cart")} />
            </div>
          )}
        </div>
        ...
        {/* Order Summary */}
        <div
          className="w-full lg:w-[400px] flex-shrink-0 p-4 rounded-xl bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 shadow-sm sticky top-6 self-start h-fit"
        >
          ...
          <div className="flex flex-col gap-4 mt-4">
            ...
            <div className="flex gap-4">
              {paymentMethods.map((method) => (
                <div
                  key={method.id}
                  onClick={() => setSelectedPayment(method.id as "cod" | "khalti")}
                  className={`flex h-32 w-36 cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border px-4 transition ${
                    selectedPayment === method.id ? "border-primary bg-primary/10" : "border-neutral-300"
                  }`}
                >
                  <Image alt={method.name} width={64} height={64} src={method.img} />
