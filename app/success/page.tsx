...
const SuccessPage = () => {
    ...
    const addOrderMutation = useMutation({
        ...
        onMutate: async (newOrder: CreateOrderInput) => {
            ...
            queryClient.setQueryData<CartItem[]>(
                ['cartItems'],
                old => old?.filter(item => !productIds.includes(item.productId)) ?? []
            );
        ...
        onError: (_error, _variables, context) => {
            ...
            dispatch(showNotification({ message: 'Failed to create order. Redirecting home.', type: 'error' }));
            setTimeout(() => {
                router.replace('/');
            }, 2000);
        },
    ...
    const processOrder = useCallback(() => {
        ...
        try {
            ...
            dispatch(showNotification({ message: 'Invalid order data. Redirecting to home.', type: 'error' }));
            setTimeout(() => router.replace('/'), 2000);
        }
